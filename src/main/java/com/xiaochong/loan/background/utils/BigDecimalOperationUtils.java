 ...
 * 确的浮点数运算，包括加减乘除和四舍五入。
 */
public class BigDecimalOperationUtils{ //默认除法运算精度
    private static final int DEF_DIV_SCALE = 10; //这个类不能实例化
    private BigDecimalOperationUtils(){
    }
    /**
     * 提供精确的加法运算。
     * @param v1 被加数
    ...
    public static BigDecimal sub(Integer v1,BigDecimal v2){
        BigDecimal i1 = new BigDecimal(Integer.toString(v1));
        return i1.subtract(v2);
    }
    /**
     * 提供精确的乘法运算。
     * @param v1 被乘数
    ...
    public static BigDecimal divBigDecimal(BigDecimal v1,Integer v2,int scale){
        if(scale<0){
            throw new IllegalArgumentException(
                    "The scale must be a positive integer or zero");
        }
