...
@Component
public class ReportComponentLog implements AsyncService {

    private Logger logger = LoggerFactory.getLogger(ReportComponentLog.class);
    ...
    @Autowired
    private ReportUserInfoDao reportUserInfoDao ;
    @Autowired
    private ReportEducationInfoDao reportEducationInfoDao ;
    @Autowired
    private ReportZhimaInfoDao reportZhimaInfoDao ;
    @Autowired
    private ReportZhimaExtendDao reportZhimaExtendDao;
    @Autowired
    private ContactInfoDao contactInfoDao ;
    @Autowired
    private ReportContactRecordDao reportContactRecordDao ;
    @Autowired
    private ReportCallRecordDao reportCallRecordDao ;
    @Autowired
    private ReportTongdunRuleDao reportTongdunRuleDao ;
    @Autowired
    private ReportLoanInfoDao reportLoanInfoDao;
    @Autowired
    private ReportBlackInfoDao reportBlackInfoDao ;
    @Autowired
    ...
    @Autowired
    private ReportYysReportDao reportYysReportDao ;
    @Autowired
    ...
    @Autowired
    private ReportLbsDao reportLbsDao ;
    @Autowired
    ...
    public Object invoke(Object data) {
        logger.info("报告异步任务开始,traceId:{}", LogTrace.getTrace().getTraceId());
        AkkaRiskOrderParamsVo paramsVo = (AkkaRiskOrderParamsVo) data;
        ...
        if (Objects.equals(creditReport.getCode(), "100")) {
            ...
            String message = creditReport.getMessage();
            logger.error("risk报告获取失败：code" + creditReport.getCode() + ",message：" + message, new Exception("报告异步失败：{}"+ message));
        }
        return data;
    }
}
